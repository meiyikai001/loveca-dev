...
const CONTROL = [
  ...
  '带 followUpTargetObjectId 的起动候选已经预选下一步的已知目标；目标明确时优先选对应候选，仍只返回一个 actionRef。服务端支付费用后重新核验窗口与候选，有新信息、新触发或目标变化就取消预选并重新询问，不跨过公开展示。普通起动候选仍可使用；预选不代表回收已完成，以后续真实观察为准。',
