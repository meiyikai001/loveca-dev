...
AI 在表演／LIVE 结果阶段机械确认规则步骤或放置唯一成功 LIVE 前，每个窗口停留 1.8 秒，给真人查看公开判定信息。临时 deadline 绑定当前任务，重复轮询不延长，窗口失效或对局结束即取消；驱动在权威队列外等待，不增加模型调用。已有公开卡牌／效果展示计时继续沿用原 deadline，不再叠加这段停留；模型抉择与其他阶段不额外延迟。
